...
def Get_BasecallN(attr):
    ...
    where instanceN is always the last token in the second part of the attribute path.
    '''
    instanceN = 'NA'
    keypartL = attr.split('/')
    if len(keypartL) >= 2: 
...
def Extract_Expt_Data(args, P, mylogger, myhandler, processname, exptid, exptdir, exptinstanceN, constD, fp):
  ...
  # Iterate across all FAST5 files
    ...
    for fast5 in passL:
        ...
        if fcnt == maxfiles:
            break
        Extract_Fast5_Data(args, P, mylogger, exptid, os.path.join(passdir, fast5), 'pass', fp, constD, exptinstanceN)
    ...
    for fast5 in failL:
        ...
        if fcnt == maxfiles:
            break
        Extract_Fast5_Data(args, P, mylogger, exptid, os.path.join(faildir, fast5), 'fail', fp, constD, exptinstanceN)
